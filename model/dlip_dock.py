...
class Blip2Qformer(Blip2Base):
    ...
    def docking_loss(self, holo_distance_predict, cross_distance_predict, holo_distance_target, distance_target,
        ...
        if not self.training:
            ...
            holo_distance_predict_train = holo_distance_predict * holo_distance_mask.float()

            holo_distance_loss_per_sample = F.mse_loss(
                holo_distance_predict_train.float(),
                holo_distance_target.float(),
                reduction='none'
            )
            masked_loss = holo_distance_loss_per_sample * holo_distance_mask


            num_valid_samples = holo_distance_mask.sum(dim=(1, 2))

            num_valid_samples[num_valid_samples == 0] = 1


            average_loss_per_sample = masked_loss.sum(dim=(1, 2)) / num_valid_samples
            holo_distance_loss_per_sample = average_loss_per_sample.view(-1, 1)

            distance_predict = cross_distance_predict * distance_mask.float()
            distance_loss_per_sample = F.mse_loss(
                distance_predict.float(), distance_target.float(), reduction='none'
            )
            masked_loss = distance_loss_per_sample * distance_mask

            num_valid_samples = distance_mask.sum(dim=(1, 2))
            num_valid_samples[num_valid_samples == 0] = 1


            distance_loss_per_sample = masked_loss.sum(dim=(1, 2)) / num_valid_samples
            distance_loss_per_sample = distance_loss_per_sample.view(-1, 1)
            loss_per_sample = distance_loss_per_sample + 3*holo_distance_loss_per_sample

            return loss_per_sample, distance_loss_per_sample, holo_distance_loss_per_sample
        distance_mask = distance_target.ne(0)  # 0 for padding, BOS and EOS
        # 0 is impossible in the cross distance matrix, all the relevant cross distances are kept
        if self.args.dist_threshold > 0:
            distance_mask &= (
    ...
    def forward(self, batch):
        ...
        batch_size = mol_batch[0].size(0)
        score_label = mol_batch[-1].view(batch_size, -1).contiguous().float()
        return loss, cross_loss, holo_loss, holo_distance_predict, cross_distance_predict, score_label
