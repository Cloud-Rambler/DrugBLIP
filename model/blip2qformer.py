...
class Blip2Qformer(Blip2Base):
    ...
    def __init__(
        ...
        self.lm = lm
        self.args = args
        self.tokenizer = self.init_tokenizer(model_name=bert_name)
        ...
        self.Qformer, self.query_tokens = self.init_Qformer(bert_name, num_query_token, self.pkt_encoder.num_features,
    ...
    def forward(self, batch):
        ## for 3d forward
        device = self.device
        graph_batch, text_batch, mol_batch = batch
        batch_node, batch_mask = self.pkt_encoder(*graph_batch)
        if self.args.use_mol_3d:
            batch_mol, batch_mol_mask = self.mol_encoder(*mol_batch[:3])
        if not self.tune_gnn:
            batch_node = batch_node.detach()
            if self.args.use_mol_3d:
                batch_mol = batch_mol.detach()
        batch_size = batch_node.shape[0]
        batch_node = self.ln_pkt(batch_node)
        if self.args.use_mol_3d:
            batch_mol = self.ln_mol(batch_mol)
            mol_feats = batch_mol[:,0,:]
            mol_feats = self.mol_proj(mol_feats)
        query_tokens = self.query_tokens.expand(batch_node.shape[0], -1, -1)
        query_output = self.Qformer.bert(
            query_embeds=query_tokens,
            encoder_hidden_states=batch_node,
        ...
        graph_feats = self.graph_proj(query_output.last_hidden_state)  # shape = [B, num_q, D]
        text_output = self.Qformer.bert(text_batch.input_ids, attention_mask=text_batch.attention_mask,
                                        return_dict=True)  # shape = [B, n_max, D]
        text_feats = self.text_proj(text_output.last_hidden_state[:, 0, :])
        if self.args.use_mol_3d:
            text_feats += mol_feats

        text_feats, graph_feats = F.normalize(text_feats, p=2, dim=-1), F.normalize(graph_feats, p=2, dim=-1)
        text_feats_all, graph_feats_all = pl_concat_all_gather(text_feats), pl_concat_all_gather(
            graph_feats)  # shape = [B * num_gpus, D]
        sim_g2t, sim_t2g, loss_gtc = self.contrast_global(graph_feats, text_feats, graph_feats_all, text_feats_all,
                                                          return_sim=True)
    ...
    def forward_v3(self, batch):
        ...
        text_output = self.Qformer.bert(text_tokens.input_ids, attention_mask=text_tokens.attention_mask,
                                        return_dict=True)  # shape = [B, n_max, D]
        text_feats = self.text_proj(text_output.last_hidden_state[:, 0, :])
    ...
    def graph_forward(self, graph):
        if self.args.use_3d:
            batch_node, batch_mask = self.pkt_encoder(*graph)
        else:
            batch_node, batch_mask = self.pkt_encoder(graph)
        batch_node = self.ln_pkt(batch_node)
        query_tokens = self.query_tokens.expand(batch_node.shape[0], -1, -1)
