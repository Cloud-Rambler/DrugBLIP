...
class TransformerEncoderWithPair(nn.Module):
    ...
    def forward(
        ...
        pocket_x: Optional[torch.Tensor] = None,
        ...
        def fill_attn_mask(attn_mask, padding_mask, fill_val=float("-inf"), seq_len=seq_len):
        ...
        if pocket_x is not None and pocket_padding_mask is not None:
            pocket_attn_bias, pocket_padding_mask = fill_attn_mask(pocket_attn_bias, pocket_padding_mask,seq_len=pocket_x.size(1))

        for i in range(len(self.layers)):
            x, attn_mask, _ = self.layers[i](
                x, padding_mask=padding_mask, attn_bias=attn_mask, return_attn=True,
                pocket_x=pocket_x, pocket_padding_mask=pocket_padding_mask
            )
